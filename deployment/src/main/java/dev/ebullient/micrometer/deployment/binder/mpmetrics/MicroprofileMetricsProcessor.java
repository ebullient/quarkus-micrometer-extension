...
import dev.ebullient.micrometer.runtime.binder.microprofile.MpMetricsRecorder;
...
public class MicroprofileMetricsProcessor {
        ...
        return new UnremovableBeanBuildItem(new Predicate<BeanInfo>() {
            ...
            public boolean test(io.quarkus.arc.processor.BeanInfo beanInfo) {
                if (beanInfo.hasType(MetricDotNames.METRIC)) {
                    if (beanInfo.hasType(MetricDotNames.ANNOTATED_GAUGE_ADAPTER)
                            || beanInfo.isProducerMethod()
                            || beanInfo.isProducerField()) {
                        return true;
                    }
                }
                return false;
    ...
    void beanDeploymentValidator(MpMetricsRecorder recorder,
            RootMeterRegistryBuildItem rootMeterRegistry,
        ...
        for (BeanInfo bean : validationPhase.getContext().beans().producers().withBeanType(MetricDotNames.METRIC)) {
            if (bean.getImplClazz() == null) {
                continue;
            }
            if (bean.getDeclaringBean() != null && bean.getDeclaringBean().hasType(MetricDotNames.INJECTED_METRIC_PRODUCER)) {
                continue;
            }

            // Find post-transformation annotations with fixed up data
            AnnotationTarget target = bean.getTarget().get();
            AnnotationInstance metricAnnotation = annotationStore.getAnnotation(target, MetricDotNames.METRIC_ANNOTATION);

            recorder.registerMetricFromProducer(rootMeterRegistry.getValue(),
                    bean.getIdentifier(),
                    bean.getImplClazz().toString(),
                    metricAnnotation.value("name").asString(),
                    metricAnnotation.value("tags").asStringArray(),
                    metricAnnotation.value("description").asString(),
                    metricAnnotation.value("unit") == null ? null : metricAnnotation.value("unit").asString());
        }
    }
