...
public class InjectedMetricProducer {
    ...
    Counter getCounter(InjectionPoint ip) {
        ...
        Object o = annotationMeters.computeIfAbsent(metric,
                k -> new CounterAdapter(registerMeter(micrometerCounter, metric)));
    ...
    Histogram getHistogram(InjectionPoint ip) {
        Metric metric = ip.getAnnotated().getAnnotation(Metric.class);
        Object o = annotationMeters.computeIfAbsent(metric,
                k -> new HistogramAdapter(registerMeter(micrometerSummary, metric)));
    ...
    Meter getMeter(InjectionPoint ip) {
        Metric metric = ip.getAnnotated().getAnnotation(Metric.class);
        Object o = annotationMeters.computeIfAbsent(metric,
                k -> new MeterAdapter(registerMeter(micrometerCounter, metric)));
    ...
    SimpleTimer getSimpleTimer(InjectionPoint ip) {
        Metric metric = ip.getAnnotated().getAnnotation(Metric.class);
        Object o = annotationMeters.computeIfAbsent(metric,
                k -> new TimerAdapter(registerMeter(micrometerTimer, metric), registry));
    ...
    <T> T registerMeter(Class<T> type, Metric metric) {
        if (micrometerCounter.equals(type)) {
            log.debugf("Counter for %s", metric);
        ...
        if (micrometerSummary.equals(type)) {
            log.debugf("DistributionSummary for %s", metric);
            return type.cast(io.micrometer.core.instrument.DistributionSummary.builder(metric.name())
                    .description(metric.description())
                    .tags(metric.tags())
                    .register(registry));
        }
        if (micrometerTimer.equals(type)) {
