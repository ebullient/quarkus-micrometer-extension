...
class MpMetricsRegistry {
    private static final Logger log = Logger.getLogger(MpMetricsRegistry.class);

    // Store class references with proper type capture
    static final Class<io.micrometer.core.instrument.Counter> micrometerCounter = io.micrometer.core.instrument.Counter.class;
    static final Class<io.micrometer.core.instrument.DistributionSummary> micrometerSummary = io.micrometer.core.instrument.DistributionSummary.class;
    static final Class<io.micrometer.core.instrument.Timer> micrometerTimer = io.micrometer.core.instrument.Timer.class;

    Map<MetricRegistry.Type, MetricRegistryAdapter> registries = new ConcurrentHashMap<>(3);

    @Produces
    @Singleton
    public MetricRegistryAdapter produceRegistry(MeterRegistry registry) {
        return registries.computeIfAbsent(MetricRegistry.Type.APPLICATION, k -> new MetricRegistryAdapter(k, registry));
    }
    ...
    public MetricRegistry produceBaseRegistry(MeterRegistry registry) {
        return registries.computeIfAbsent(MetricRegistry.Type.BASE, k -> new MetricRegistryAdapter(k, registry));
    }
    ...
    public MetricRegistry produceVendorRegistry(MeterRegistry registry) {
        return registries.computeIfAbsent(MetricRegistry.Type.VENDOR, k -> new MetricRegistryAdapter(k, registry));
    }

    static class MetricRegistryAdapter extends MetricRegistry {
        final Type type;
        ...
        <T> GaugeAdapter<Double> internalGauge(MpMetadata metadata, MetricDescriptor id, T obj, ToDoubleFunction<T> f) {
            GaugeAdapter<Double> result = checkCast(GaugeAdapter.class, metadata,
                    constructedMeters.computeIfAbsent(id, k -> new GaugeAdapter.DoubleFunctionGauge<T>(obj, f)));
            return result.register(metadata, id, registry);
        }
    ...
    static <T> T checkCast(Class<T> type, MpMetadata metadata, MeterHolder o) {
        try {
            return type.cast(o);
